...
fixture {
    ...
    (1..10).each {count ->
        ...
        if (!AuthUser.findByUsername(username)) {
            "$username"(AuthUser, username: username, name: username, password: defaultPassword, enabled: true)
        }
    }
...
post {
    ...
    (1..10).each {count ->
        def u = AuthUser.findByUsername("user${count}")
    ...
    (1..5).each {count ->
        def u = AuthUser.findByUsername("admin${count}")
        if (!UserRoleMapper.findByUserAndRole(u, userRole)) {
            UserRoleMapper.create u, userRole
